# CloudMeter US 雲端抄表系統操作手冊

本操作手冊將帶領您了解如何使用 CloudMeter US 的各項核心功能。以下依據不同使用者情境，詳細介紹對應的操作流程。
...
### 情境一：新客戶試算與註冊
**情境描述**：身為新客戶，您想了解若導入 CloudMeter US 系統所需花費的年度預算，並註冊專屬帳號。
**操作流程**：
...
4. **註冊資料**：依序輸入「公司名稱」、「管理員信箱」以及「設定密碼」，點選「註冊並前往付款」。

![建立企業管理員帳號](./images/2_register.png)
...
### 情境三：日常帳務與狀態管理
**情境描述**：身為企業管理員，您需要檢視系統授權狀態與過往帳單紀錄。
**操作流程**：
1. **登入系統**：點擊首頁右上方的「登入」，輸入管理員信箱與密碼。
2. **預覽儀表板**：登入後可於上方區塊確認目前訂閱狀態、可用授權數及下次帳單日期。

![企業後台儀表板](./images/4_client_dashboard.png)
...
**操作流程**：
1. **進入擴充功能**：在企業後台點擊「快速操作」列表中的「擴充表位數量」。
...
### 情境六：財務收益與客戶管理
**情境描述**：您是 CloudMeter 後台營運人員，需要檢視系統財務狀態及用戶管理。
**操作流程**：
1. **切換為員工**：於系統右上角導覽列點擊「切換至員工後台」。
2. **財務收益儀表板**：登入後預設進入「財務收益」總覽，查看營業額統計。

![員工端財務收益儀表板](./images/7_admin.png)
